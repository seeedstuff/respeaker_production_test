...
volatile uint32_t sendUpradeCodeTime;
...
const char *str_bootSelection = "9: Load Boot Loader code then write to Flash via TFTP.";
...
// Refference board Insert Detect Pin 
const uint8_t boardInsertDetectPin = 2;
volatile int boardInsertDetectPinLastState;
volatile bool is_UARTOpen = false;
...
volatile int main_task_id = main_task_upgrade_from_bootloader;
// volatile int main_task_id = nothingToDo;
...
void MT7688_send_fw_upgrade(int cnt)
{
  for(int i = 0; i < cnt; i++) {
    Serial1.write('5');  
    // delay(50);  
...
void arduino_gpio_test(void)
{
  ...
  for(uint16_t i = 0; i < group_len; i++) {
...
void callback()
{
    if(LOW == digitalRead(boardInsertDetectPin) && HIGH == boardInsertDetectPinLastState)
    {
        // In this situation to know Board has been inserted.
        boardInsertDetectPinLastState = LOW;
        // Serial.println("boardInsertDetectPin is LOW...");
    } 
    else if( HIGH == digitalRead(boardInsertDetectPin) && LOW == boardInsertDetectPinLastState )
    {   
        boardInsertDetectPinLastState = HIGH;
        // Serial.println("boardInsertDetectPin is HIGH...");
    }

}

bool is_boardInserted()
{
    return boardInsertDetectPinLastState == LOW;
}
...
void setup (void)
{
  ...
  // Timing interrupt to read boardInsertDetectPin
  pinMode(boardInsertDetectPin, INPUT);
}
...
void loop(void)
{
  ...
  sendUpradeCodeTime = millis();
  respeaker.pixels().clear();
  respeaker.pixels().update();

  while(HIGH == digitalRead(boardInsertDetectPin)) 
  {
    if(!is_UARTOpen) {
        is_UARTOpen = true;
        Serial1.begin(57600);
    ...
    switch(main_task_id) {
        ...
        case main_task_upgrade_from_bootloader:
            ret = parseStrFromSerial(str_bootSelection, strlen(str_bootSelection));
            if( 200 < millis() - sendUpradeCodeTime ){
                sendUpradeCodeTime = millis();
                if((HIGH == digitalRead(boardInsertDetectPin)) && 
                (main_task_id == main_task_upgrade_from_bootloader)){
                    Serial1.write('5');
                }
            }
            if(1 == ret) {
                // MT7688_send_fw_upgrade(40);
            ...
            if(5000 < millis() - UARTtimeStart) {
                Serial.println("\n\r\n\rNot received boot selection string...\n\r");
                rgb_led_state.color = COLOR_RED;
                rgb_led_state.number = 1;
        ...
        case main_task_fw_writting:
            ...
            if(1 ==ret) {
                Serial.println("FW upgrade done...");
            ...
            if (300000 < millis() - UARTtimeStart) {
                Serial.println("FW upgrade failed...");
                rgb_led_state.color = COLOR_RED;
                rgb_led_state.number = 3;
                main_task_id = main_task_end;
            }
